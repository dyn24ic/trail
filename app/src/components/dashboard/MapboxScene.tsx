...
export default function MapboxScene({
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        paint: { 'line-color': '#00FF88', 'line-width': 10, 'line-opacity': 0.18, 'line-blur': 8 },
      });
      map.addLayer({
  ...
  useEffect(() => {
    ...
    if (!map) return;
    const apply = () => {
      map.easeTo({ center: [flyTo.lon, flyTo.lat], zoom: flyTo.zoom, pitch: 55, bearing: -20, duration: 800 });
    };
  ...
  const setupBoxZoom = useCallback(() => {
    ...
    const onMouseUp = (e: MouseEvent) => {
      ...
      if (dx > 8 && dy > 8) {
        ...
        map.fitBounds([map.unproject([x0, y1]), map.unproject([x1, y0])], { padding: 20, duration: 800 });
      }
  ...
  useEffect(() => {
    ...
    const addLayers = () => {
      ...
      map.addSource('danger-zones', { type: 'geojson', data: { type: 'FeatureCollection', features } });
      map.addLayer({
        ...
        paint: {
          'fill-color': [
            'interpolate', ['linear'], ['get', 'hazard_score'],
  ...
  useEffect(() => {
    ...
    map.setLayoutProperty('danger-zones-fill', 'visibility', layers.zones ? 'visible' : 'none');
  }, [layers.zones]);
  ...
  useEffect(() => {
    ...
    map.setLayoutProperty('hotspots-circle', 'visibility', layers.hotspots ? 'visible' : 'none');
  }, [layers.hotspots]);
  ...
  useEffect(() => {
    ...
    const featureCollection: GeoJSON.FeatureCollection = {
      ...
      features: bleScanners.map(s => ({
        type: 'Feature' as const,
        properties: { type: s.type, priority: s.priority, label: s.label, name: s.name },
    ...
    const apply = () => {
      if (map.getSource(SRC)) {
        (map.getSource(SRC) as mapboxgl.GeoJSONSource).setData(featureCollection);
        return;
      }
      map.addSource(SRC, { type: 'geojson', data: featureCollection });
      map.addLayer({
        id: 'ble-scanners-glow', type: 'circle', source: SRC,
        paint: { 'circle-radius': 20, 'circle-color': '#a855f7', 'circle-opacity': 0.12, 'circle-stroke-width': 0 },
      });
      map.addLayer({
        id: 'ble-scanners-circle', type: 'circle', source: SRC,
        paint: {
          'circle-radius': ['case', ['==', ['get', 'type'], 'trailhead'], 11, ['==', ['get', 'type'], 'junction'], 8, 7],
          'circle-color': ['case', ['==', ['get', 'type'], 'trailhead'], '#a855f7', ['==', ['get', 'type'], 'junction'], '#7c3aed', '#f97316'],
          'circle-stroke-color': '#ffffff', 'circle-stroke-width': 1.5, 'circle-opacity': 0.95,
        },
      });
      map.addLayer({
        id: 'ble-scanners-label', type: 'symbol', source: SRC,
        layout: {
        ...
        paint: { 'text-color': '#ffffff', 'text-halo-color': 'rgba(0,0,0,0.8)', 'text-halo-width': 1 },
      });
    };
  ...
  useEffect(() => {
    ...
    map.addSource('drone-lines-src', {
      ...
      data: {
        ...
        features: droneData.map(d => ({
          type: 'Feature' as const,
          properties: {},
          geometry: { type: 'LineString' as const, coordinates: [[d.hubLon, d.hubLat], [d.lon, d.lat]] },
        })),
      },
    });
    map.addLayer({
      id: 'drone-lines', type: 'line', source: 'drone-lines-src',
      paint: { 'line-color': '#FFC107', 'line-width': 1, 'line-dasharray': [4, 3], 'line-opacity': 0.55 },
    ...
    droneData.forEach(drone => {
      ...
      hubEl.style.cssText = 'width:10px;height:10px;background:rgba(255,193,7,0.5);border:1.5px solid #FFC107;transform:rotate(45deg);cursor:pointer;';
      if (!showDrones) hubEl.style.display = 'none';
      hubMarkerEls.current.push(new mapboxgl.Marker({ element: hubEl }).setLngLat([drone.hubLon, drone.hubLat]).addTo(map));

      // Drone — pulsing ring + ✈ + altitude badge
      const droneEl = document.createElement('div');
      droneEl.style.cssText = 'position:relative;width:32px;height:32px;cursor:pointer;';
      droneEl.innerHTML = `
      ...
      droneMarkerEls.current.push(
        new mapboxgl.Marker({ element: droneEl, anchor: 'center' })
          ...
          .setPopup(new mapboxgl.Popup({ offset: 20, closeButton: false })
            .setHTML(`<div style="font-family:monospace;font-size:11px;color:#b0d8c8;"><b style="color:#FFC107;">${drone.id}</b><br/>${drone.label}<br/>Alt: ${drone.alt}m AGL</div>`))
          .addTo(map)
      );
    });

    return () => {
      droneMarkerEls.current.forEach(m => m.remove()); droneMarkerEls.current = [];
      hubMarkerEls.current.forEach(m => m.remove()); hubMarkerEls.current = [];
  ...
  useEffect(() => {
    ...
    droneMarkerEls.current.forEach(m => { m.getElement().style.display = show ? '' : 'none'; });
    hubMarkerEls.current.forEach(m => { m.getElement().style.display = show ? '' : 'none'; });
    const map = mapRef.current;
    if (map?.getLayer('drone-lines')) map.setLayoutProperty('drone-lines', 'visibility', show ? 'visible' : 'none');
  }, [layers.drones]);
  ...
  useEffect(() => {
    ...
    incidentMarkerEls.current.forEach(m => { m.getElement().style.display = show ? '' : 'none'; });
  }, [layers.incidents]);
  ...
  useEffect(() => {
    ...
    function addRoute() {
      ...
      if (hybridRoute.roadPath.length >= 2) {
        map.addSource('route-road-src', {
          ...
          data: {
            ...
            geometry: { type: 'LineString', coordinates: hybridRoute.roadPath.map(p => [p.lon, p.lat]) },
          },
        });
        map.addLayer({
          id: 'route-road', type: 'line', source: 'route-road-src',
      ...
      map.addLayer({
        id: 'route-mountain', type: 'line', source: 'route-mountain-src',
        layout: { 'line-cap': 'round', 'line-join': 'round' },
        paint: {
          'line-width': 5,
          'line-gradient': [
      ...
      map.addLayer({
        id: 'route-stop', type: 'circle', source: 'route-stop-src',
        paint: { 'circle-radius': 10, 'circle-color': '#f97316', 'circle-stroke-width': 3, 'circle-stroke-color': '#ffffff' },
      });
      ...
      map.addLayer({
        id: 'route-victim', type: 'circle', source: 'route-victim-src',
        paint: { 'circle-radius': 12, 'circle-color': '#ef4444', 'circle-stroke-width': 3, 'circle-stroke-color': '#ffffff' },
      });
  ...
  useEffect(() => {
    mapRef.current?.getCanvas().style.setProperty('cursor', hikeMode ? 'crosshair' : '');
  }, [hikeMode]);
  ...
  useEffect(() => {
    ...
    const lineGeoJSON: GeoJSON.FeatureCollection = nodes.length < 2
    ...
    const apply = () => {
      if (map.getSource(lineSrcId)) {
        (map.getSource(lineSrcId) as mapboxgl.GeoJSONSource).setData(lineGeoJSON);
        (map.getSource(nodesSrcId) as mapboxgl.GeoJSONSource).setData(nodesGeoJSON);
        return;
      }
      ...
      map.addLayer({
        id: casingLayId, type: 'line', source: lineSrcId,
        paint: { 'line-color': '#000000', 'line-width': 6 },
      });
      map.addLayer({
        id: lineLayId, type: 'line', source: lineSrcId,
        paint: { 'line-color': '#00ffc8', 'line-width': 3, 'line-dasharray': [2, 1] },
      });
      map.addLayer({
        id: nodesLayId, type: 'circle', source: nodesSrcId,
        paint: {
          'circle-radius': ['case', ['==', ['get', 'nodeType'], 'start'], 9, ['==', ['get', 'nodeType'], 'end'], 9, 6],
          'circle-color': ['case', ['==', ['get', 'nodeType'], 'start'], '#22c55e', ['==', ['get', 'nodeType'], 'end'], '#ef4444', '#00ffc8'],
