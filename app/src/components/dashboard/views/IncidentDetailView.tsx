...
const CAT_LABEL: Record<string, string> = {
  ...
  trail_closure: "✕ Closure",
  trail_reroute: "↺ Reroute",
  risk_alert: "▲ Alert",
};
...
function stageIdx(s: IncidentStatus) {
    ...
    <div className="incident-detail">
      ...
      <div className="incident-detail-header">
        <div>
          ...
          <div className="incident-detail-type" style={{ color: accentColor }}>
            {inc.triggerType}
          </div>
        </div>
        <div
        ...
        >
          <span
            className="incident-detail-status"
            style={{ borderColor: accentColor, color: accentColor }}
          >
            {inc.status}
          </span>
          {loading && (
            <span style={{ fontSize: "0.55rem", color: "var(--db-muted)" }}>
              ↻ polling
            </span>
          )}
        </div>
      ...
      <div className="incident-detail-pipeline">
        ...
        <div className="pipeline-stages">
          {STAGES.map((s, i) => {
            ...
            const current = i === cur;
            const color = !done
              ? "rgba(220,232,240,0.08)"
              : s === "Closed"
                ? "var(--db-green)"
                : s === "Triggered" || s === "Searching"
                  ? "var(--db-red)"
                  : "var(--db-amber)";
            return (
              <div key={s} className="pipeline-stage">
                ...
                <div
                  ...
                  style={{ color: done ? color : "var(--db-muted)" }}
                >
                  {STAGE_LABELS[s]}
                </div>
              </div>
        ...
        <div className="panel-section">
          ...
          {inc.searchZones.map((z, i) => (
            <div key={i} className="zone-card">
              <div className={`zone-rank ${["r1", "r2", "r3"][i] ?? "r3"}`}>
                {i + 1}
              </div>
        ...
        <div className="panel-section">
          <div className="panel-heading">
            ...
            <span
              ...
              style={{
                color: inc.droneResult.victimFound
                  ? "var(--db-green)"
                  : "var(--db-red)",
              }}
            >
          ...
          <div className="incident-meta" style={{ padding: "10px 20px" }}>
            {inc.droneResult.victimFound && (
              <>
                <strong>Victim</strong> {fmtCoord(inc.droneResult.victimLat)},{" "}
                {fmtCoord(inc.droneResult.victimLng)}
                <br />
                <strong>Confidence</strong>{" "}
                {(inc.droneResult.confidence * 100).toFixed(0)}%<br />
              </>
            )}
            <strong>Scan time</strong> {inc.droneResult.scanDurationSeconds}s
            {inc.droneResult.imageUrl && (
              <>
                <br />
                <strong>Feed</strong>{" "}
                <span style={{ color: "var(--db-blue)" }}>● CAPTURED</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className="panel-section">
          <div className="panel-heading">
            ...
            <span
              className="panel-badge"
              style={{
                color: SEV_COLOR[inc.triage.severity] ?? "var(--db-muted)",
              }}
            >
          ...
          <div className="incident-meta" style={{ padding: "10px 20px" }}>
            <strong>Injury</strong> {inc.triage.injuryType}
            <br />
            <strong>Conscious</strong>{" "}
            {inc.triage.consciousAndResponsive ? "Yes" : "No"}
            <br />
            <strong>Response</strong> {inc.triage.recommendedResponse}
            <br />
            <strong>Urgency</strong> {inc.triage.estimatedMedicalUrgencyMinutes}{" "}
            min
          </div>
        </div>
      )}
        ...
        <div className="panel-section">
          <div className="panel-heading">
            ...
            <span className="panel-badge">
              {inc.route.accessType} · {inc.route.totalEtaMinutes} min
            </span>
          </div>
          <div className="incident-meta" style={{ padding: "8px 20px 0" }}>
            <strong>Distance</strong>{" "}
            {(inc.route.totalDistanceMeters / 1000).toFixed(1)} km
          </div>
      ...
      {inc.status === "Closed" && (
        <>
          <ExternalReportsSection
            ...
            onSaved={(updated) =>
              setIncident((prev) =>
                prev ? { ...prev, externalReports: updated } : prev,
              )
            }
          />
            ...
            onGenerate={async () => {
              ...
              try {
                const res = await fetch(
                  `/api/incidents/${inc.id}/ai-recommendation`,
                  { method: "POST" },
                );
                if (res.ok) {
                  const reco: AiRecommendation = await res.json();
                  setIncident((prev) =>
                    prev ? { ...prev, aiRecommendation: reco } : prev,
                  );
                }
              } finally {
                setAiLoading(false);
...
function ExternalReportsSection({
  ...
  const [ambulanceForm, setAmbulanceForm] = useState<
    Omit<AmbulanceReport, "submittedAt">
  >({
    paramedicName: "",
    vehicleId: "",
    treatmentGiven: "",
  ...
  const btnStyle = (active?: boolean): React.CSSProperties => ({
    ...
    color: active ? "var(--db-blue)" : "var(--db-muted)",
    ...
    <div className="panel-section">
      ...
      {externalReports?.police && (
        ...
        >
          ...
          >
            POLICE · {externalReports.police.officerName} #
            {externalReports.police.badgeNumber}
          </div>
          <div style={{ color: "var(--db-text)", lineHeight: 1.5 }}>
            {externalReports.police.description}
          </div>
          {externalReports.police.crimeInvolved && (
            <div
              style={{
                color: "var(--db-amber)",
                marginTop: "4px",
      ...
      {externalReports?.ambulance && (
        ...
        >
          ...
          >
            AMBULANCE · {externalReports.ambulance.paramedicName} ·{" "}
            {externalReports.ambulance.vehicleId}
          </div>
          <div style={{ color: "var(--db-text)", lineHeight: 1.5 }}>
            {externalReports.ambulance.treatmentGiven}
          </div>
          {externalReports.ambulance.hospitalDestination && (
            <div style={{ color: "var(--db-muted)", marginTop: "3px" }}>
              → {externalReports.ambulance.hospitalDestination}
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setPoliceForm((f) => ({ ...f, officerName: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setPoliceForm((f) => ({ ...f, badgeNumber: e.target.value }))
                }
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              ...
              value={policeForm.description}
              onChange={(e) =>
                setPoliceForm((f) => ({ ...f, description: e.target.value }))
              }
            />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <input
              type="checkbox"
              id="crimeInvolved"
              checked={policeForm.crimeInvolved}
              onChange={(e) =>
                setPoliceForm((f) => ({
                  ...f,
                  crimeInvolved: e.target.checked,
                }))
              }
            />
            <label
              htmlFor="crimeInvolved"
              style={{ ...labelStyle, marginBottom: 0, cursor: "pointer" }}
            >
              Crime involvement suspected
            </label>
          </div>
          <button
            style={{ ...btnStyle(true), alignSelf: "flex-start" }}
            onClick={submitPolice}
            disabled={saving === "police"}
          >
        ...
        >
          ...
          >
            <div>
              ...
              <input
                style={inputStyle}
                value={ambulanceForm.paramedicName}
                onChange={(e) =>
                  setAmbulanceForm((f) => ({
                    ...f,
                    paramedicName: e.target.value,
                  }))
                }
              />
            </div>
            <div>
              ...
              <input
                style={inputStyle}
                value={ambulanceForm.vehicleId}
                onChange={(e) =>
                  setAmbulanceForm((f) => ({ ...f, vehicleId: e.target.value }))
                }
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              style={{ ...inputStyle, height: "60px", resize: "vertical" }}
              value={ambulanceForm.treatmentGiven}
              onChange={(e) =>
                setAmbulanceForm((f) => ({
                  ...f,
          ...
          <div>
            ...
            <input
              style={inputStyle}
              value={ambulanceForm.hospitalDestination ?? ""}
              onChange={(e) =>
                setAmbulanceForm((f) => ({
                  ...f,
                  hospitalDestination: e.target.value || null,
                }))
              }
            />
          </div>
          <button
            style={{ ...btnStyle(true), alignSelf: "flex-start" }}
            onClick={submitAmbulance}
            disabled={saving === "ambulance"}
          >
  ...
  return (
    <div className="panel-section">
      ...
      {!recommendation && !loading && (
        ...
        >
          Generate an AI-powered recommendation using all incident data,
            ...
            {recommendation.immediateActions.map((a, i) => (
              <div
                ...
                style={{
                  ...
                  marginBottom: "4px",
                  paddingLeft: "10px",
                  borderLeft: "2px solid var(--db-amber)",
                }}
              >
                {a}
              </div>
            ...
            {recommendation.preventionMeasures.map((m, i) => (
              <div
                ...
                style={{
                  ...
                  marginBottom: "4px",
                  paddingLeft: "10px",
                  borderLeft: "2px solid var(--db-green)",
                }}
              >
                {m}
              </div>
    ...
    <>
      <div className="panel-section">
        ...
        <div className="incident-meta" style={{ padding: "10px 20px 8px" }}>
          <strong>Total response</strong> {timeline.totalResponseMinutes} min
          <br />
          <strong>Drone search</strong> {timeline.droneSearchSeconds}s<br />
          <strong>Victim found</strong> {timeline.victimFound ? "Yes" : "No"}
          {timeline.triageSeverity && (
            <>
              <br />
              <strong>Triage severity</strong> {timeline.triageSeverity}
            </>
          )}
        </div>
        <div
        ...
        >
          {(
            [
              [
                "Best zone conf.",
                `${(m.bestZoneConfidence * 100).toFixed(0)}%`,
              ],
              ...
              >
                <span
                  style={{
                    ...
                    letterSpacing: "0.08em",
                    padding: "2px 6px",
                    border: "1px solid rgba(220,232,240,0.15)",
                    color: "var(--db-text)",
                  }}
                >
