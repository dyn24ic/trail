...
class ResponderRoutingService:
  ...
  def route(
    ...
    def randomHazards: List[String] =
      rng.shuffle(possibleHazards).take(rng.nextInt(3))

    val steps = List(
      RouteStep(
        1,
        "Depart trailhead / staging area",
        distMeters * 0.15,
        (etaMin * 0.10).toInt,
        randomHazards
      ),
      RouteStep(
        2,
        "Navigate to ridge access point",
        distMeters * 0.35,
        (etaMin * 0.30).toInt,
        randomHazards
      ),
      RouteStep(
        3,
        "Approach victim GPS coordinates",
        distMeters * 0.40,
        (etaMin * 0.45).toInt,
        randomHazards
      ),
      RouteStep(
        4,
        "Stabilise and prepare for evacuation",
        distMeters * 0.10,
        (etaMin * 0.15).toInt,
        List.empty
      )
    ...
    ResponderRoute(
      ...
      notes =
        s"Route optimised for $accessType access. Distance: ${"%.1f".format(distKm)} km."
    )
